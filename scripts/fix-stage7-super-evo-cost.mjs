...
if (!source.includes(spendBefore) && !source.includes(spendAfter)) throw new Error("Stage 7 spend-cost anchor not found");
source = source.replace(spendBefore, spendAfter);

const preserveBefore = `    if (context.normalEquivalent) score -= 6;`;
const preserveAfter = `    if (context.normalEquivalent) score -= 40;`;
if (!source.includes(preserveBefore) && !source.includes(preserveAfter)) throw new Error("Stage 7 preservation anchor not found");
source = source.replace(preserveBefore, preserveAfter);

fs.writeFileSync(path, source);
console.log("Stage 7 strongly preserves Super Evo when normal Evo is equivalent.");
