...
function stage7EvolutionPrior(unit, player, opponent, superMode) {
  ...
  // Four explicit evolution intents: lethal/offense, defense, tempo swing and value.
  ...
  // Preserve scarce resources when the stronger evolution does not change the line.
  if (superMode) {
  ...
  // Pure-stat evolution on an empty enemy board is usually a resource leak.
...
`;
  ...
  for (const action of sequence) {
    ...
    if (unit && player && opponent) {
      ...
      // The evolved follower may have traded and left play. Strong action priors
      // still distinguish deliberate tactical evolution from blind resource use.
