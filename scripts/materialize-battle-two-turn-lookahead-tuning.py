...
inspect_old = '''    worstFutureScore: plan.worstFutureScore ?? null,\n    futureSamples: plan.futureSamples ?? 0\n  };'''
inspect_new = '''    worstFutureScore: plan.worstFutureScore ?? null,\n    futureSamples: plan.futureSamples ?? 0,\n    futureDiagnostics: plan.futureDiagnostics ?? []\n  };'''
if inspect_old not in text:
    raise SystemExit('inspect future diagnostics anchor missing')
text = text.replace(inspect_old, inspect_new, 1)

path.write_text(text, encoding='utf-8')
print('Battle Sim future first-action search diversified with diagnostics')
