...
const engineUrl = new URL("../js/battle-engine-v5.js", import.meta.url);
const rulesUrl = new URL("../js/battle-rules.js", import.meta.url);
const coreUrl = new URL("../js/battle-rules-core.js", import.meta.url);

let engine = await fs.readFile(engineUrl, "utf8");
let rules = await fs.readFile(rulesUrl, "utf8");
let core = await fs.readFile(coreUrl, "utf8");

function insertBefore(source, anchor, block, marker) {
  if (source.includes(marker)) return source;
  const index = source.indexOf(anchor);
  if (index < 0) throw new Error(`Missing anchor for ${marker}: ${anchor}`);
  return source.slice(0, index) + block + source.slice(index);
}

function insertAfter(source, anchor, block, marker) {
  if (source.includes(marker)) return source;
  const index = source.indexOf(anchor);
  if (index < 0) throw new Error(`Missing anchor for ${marker}: ${anchor}`);
...
// ---------------------------------------------------------------------------
// Coverage declarations. These are only safe because the runtime rules below
// are materialized and regression-tested in the same workflow before commit.
// ---------------------------------------------------------------------------
const fullOverrideAnchor = '  ["lu woh, light personified", "Storm-attack reduction Crest and Countdown are modeled"]\n]);';
const fullOverrideReplacement = `  ["lu woh, light personified", "Storm-attack reduction Crest and Countdown are modeled"],
...
engine = replaceOnce(engine, fullOverrideAnchor, fullOverrideReplacement, "v5 FULL_OVERRIDES");

// ---------------------------------------------------------------------------
// Player state: Faith + temporary leader damage cap.
// ---------------------------------------------------------------------------
engine = replaceOnce(
  engine,
...
// Enhanced modes need to remain identifiable after mode expansion.
engine = replaceOnce(
  engine,
...
// Enhanced-card event after the played follower/amulet has entered the field.
engine = insertBefore(
  engine,
...
// Permanent "can't attack" must survive turn refresh and evolution.
engine = replaceOnce(
  engine,
...
// Crest countdowns discovered in card rules.
engine = replaceOnce(
  engine,
...
// Temporary leader cap ends after the protected leader's opponent finishes.
engine = insertBefore(
  engine,
...
// Bayle: any allied follower leaving the field reduces copies in hand.
engine = insertBefore(
  engine,
...
// Engine context primitives used by exact card rules.
engine = insertAfter(
  engine,
...
// Gildaria's Rally gate must be resolved before the generic Crest parser, or
// the Crest would be granted even when Rally 20 is not met.
engine = insertAfter(
  engine,
...
// Core generic leader damage honors temporary leader caps.
core = replaceOnce(
  core,
...
// ---------------------------------------------------------------------------
// Rule layer: exact reactive events and card-specific clauses.
// ---------------------------------------------------------------------------
const oldBuffFunctionStart = rules.indexOf('export function applyBuffedFollowerEffects(context, unit, before = null) {');
const oldBuffFunctionEnd = rules.indexOf('export function applySpellPlayedEffects(context) {', oldBuffFunctionStart);
if (oldBuffFunctionStart < 0 || oldBuffFunctionEnd < 0) throw new Error("applyBuffedFollowerEffects block not found");
if (!rules.includes("// [[battle-buff-reactions-100]]")) {
  const newBuffFunction = `export function applyBuffedFollowerEffects(context, unit, before = null) {
...
`;
  rules = rules.slice(0, oldBuffFunctionStart) + newBuffFunction + rules.slice(oldBuffFunctionEnd);
}

// Crest + source entry triggers.
rules = insertAfter(
  rules,
...
rules = insertAfter(
  ...
  `
    ...
    if (name === "mars, conflagrant commander" && isOfficer) {
      const beforeUnit = { attack: unit.attack, defense: unit.defense };
      ...
      actions.push(...applyBuffedFollowerEffects(context, unit, beforeUnit));
    }
`,
  "// [[battle-entry-source-rules-100]]"
);

// Transform counts as leaving the field for Bayle.
rules = insertBefore(
  rules,
...
rules = insertBefore(
  ...
  if (cardName === "krulle, heir to unkilling") {
    ...
    if (debuff.test(text)) {
      ...
      if (targets.length && context.player.isActive && context.sourceUnit?.__krulleReactionTurn !== context.player.personalTurn) {
        context.sourceUnit.__krulleReactionTurn = context.player.personalTurn;
        const healed = healLeader(context.player, 1, context.stats, context.playerIndex);
        actions.push(`Krulle: restore ${healed} leader defense`);
      }
  ...
  if (cardName === "sofina, inspiring strength") {
    ...
    if (endDebuff.test(text)) {
      if (context.sourceUnit?.evolved || context.sourceUnit?.superEvolved) {
        ...
        for (const unit of context.opponent.board.filter(unit => unit.type === "Follower")) { unit.attack -= 1; unit.defense -= 1; unit.maxDefense -= 1; }
...
await fs.writeFile(coreUrl, core);
console.log("Battle Sim 100% coverage rules materialized.");
