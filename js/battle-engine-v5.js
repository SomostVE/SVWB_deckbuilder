...
// [[battle-fanfare-accuracy-pass-v1]]
function resolveFanfareAccuracyPass(textValue, ctx) {
  ...
  if (name === "adahime, anathema of death") text = text.replace(/Whenever another allied Abysscraft follower enters the field, give it Rush\.?/i, " ");
  if (name === "fiole, devilish matriarch") text = text.replace(/Whenever an allied Bat enters the field, give it Rush\.?/i, " ");
  ...
  if (name === "gido, leader of the pack") {
    consume(/If your leader'?s defense is 10 or less, evolve this follower\.?/i, () => { if ((Number(ctx.player.hp) || 0) <= 10 && source) evolveUnitByAbility(ctx, source, actions); });
  }
  ...
  if (name === "colette, barrage exorcist") {
    consume(/Do this 1 time:\s*["“]Deal 2 damage to a random enemy follower\.["”]\s*If there are at least 2 allied amulets on the field, do it 2 times instead\.?/i, () => {
      ...
      for (let i = 0; i < count; i += 1) {
        const pool = enemyFollowers(); if (!pool.length) break;
        const target = pool[Math.floor(ctx.rng() * pool.length)];
        damageUnit(target, 2, ctx.opponent, ctx.player, ctx, actions);
        actions.push(...cleanup(ctx.opponent, ctx.player, ctx.enemyIndex, ctx.playerIndex, ctx.stats, ctx.rng, ctx.cardMap));
      }
  ...
  if (name === "skullfane of demise") {
    consume(/Destroy all allied amulets\.\s*Deal X damage to all enemies\.\s*X is the number of amulets destroyed by this ability\.?/i, () => {
      ...
      for (const unit of targets) {
        const before = ctx.player.board.length;
        destroyObjectExact(ctx.player, ctx.opponent, unit, ctx.playerIndex, ctx.enemyIndex);
        if (ctx.player.board.length < before) destroyed += 1;
      }
  ...
  if (name === "cheretta, angelic maid") consume(/If you'?ve unlocked super-evolution, give this follower \+0\/\+3\.?/i, () => { if (superEvolutionUnlocked()) buff(source, 0, 3); });
  if (name === "vyrn, bestest pal") consume(/If you'?ve unlocked super-evolution, evolve this follower\.?/i, () => { if (superEvolutionUnlocked() && source) evolveUnitByAbility(ctx, source, actions); });
  ...
  if (name === "demon of purgatory") {
    consume(/Select 2 enemy followers on the field and deal 6 damage to them and the enemy leader\.?/i, () => {
      ...
      const leader = damageEnemyLeader(6);
      actions.push(`Demon of Purgatory: 6 damage to ${targets.length} followers and ${leader} to leader`);
    });
  }
  ...
  if (name === "beast lost to the dark") {
    consume(/Give this follower 3 random abilities from the following\.\s*1\.\s*Storm\.\s*2\.\s*Bane\.\s*3\.\s*Intimidate\.\s*4\.\s*Drain\.\s*5\.\s*Aura\.\s*6\.\s*Barrier\.?/i, () => {
      const chosen = chooseRandomDistinct(["Storm", "Bane", "Intimidate", "Drain", "Aura", "Barrier"], 3);
  ...
  if (name === "sho, reborn night king") consume(/If you'?ve unlocked super-evolution, give this follower Barrier\.?/i, () => { if (superEvolutionUnlocked()) addKeyword(source, "Barrier"); });
  ...
  if (name === "neuron disrupter") {
    consume(/Recover X play points\.\s*X is the number of other allied followers on the field\.?/i, () => {
      const x = otherAlliedFollowers().length;
      const before = ctx.player.pp;
      ctx.player.pp = Math.min(Number(ctx.player.maxPp)||0, (Number(ctx.player.pp)||0)+x);
      actions.push(`Neuron Disrupter: recover ${ctx.player.pp-before} PP`);
    });
  }
  ...
  if (name === "noah, thread of death") {
    consume(/Add 3 copies of Puppet to your hand\.\s*Give all Puppetry followers in your hand \+1\/\+0\.?/i, () => {
      ...
      if (added) ctx.stats.cardsGenerated[ctx.playerIndex] += added;
      let buffed = 0;
      for (const item of ctx.player.hand) if (item.card?.type === "Follower" && (item.card?.traits ?? []).some(value => norm(value) === "puppetry")) { item.attackBonus = (Number(item.attackBonus)||0)+1; buffed += 1; }
  ...
  if (name === "ezecrain, portent of vengeance") {
    consume(/Select 2 enemy followers on the field and deal them 4 damage\.\s*Gain 2 earth sigils\.?/i, () => {
      ...
      actions.push(`Ezecrain: 4 damage ×${targets.length}, Earth Sigils +2`);
    });
  }
  if (name === "waterbending charmwielder") {
    consume(/Deal 3 damage to 3 random enemy followers\.\s*Spellboost your hand 3 times\.?/i, () => {
      ...
      spellboostHand(ctx.player, 3, ctx.cardMap, actions);
      actions.push("Waterbending Charmwielder: Spellboost ×3");
    });
  }
  ...
  if (name === "raio, elimination manifest") {
    consume(/Reduce the cost of all followers in your deck by 3\.\s*Transform a random spell in your hand into an Ersatz Elimination\.\s*It costs 0 until the end of the turn\.?/i, () => {
      ...
      const spells = ctx.player.hand.filter(item => item.card?.type === "Spell");
      const target = spells.length ? spells[Math.floor(ctx.rng()*spells.length)] : null;
      ...
      if (target && next) {
        const beforeName = target.card.name;
        transformHandInstance(target, next);
        const current = costOf(target);
        target.costDelta = (Number(target.costDelta)||0)-current;
        target.raioTemporaryZeroCost = current;
        actions.push(`Raio: transform ${beforeName} into Ersatz Elimination at 0 PP this turn`);
      }
      actions.push("Raio: follower deck costs -3");
    });
  }

  if (name === "ignominious samurai") consume(/If you'?ve unlocked super-evolution, give this follower Bane\.?/i, () => { if (superEvolutionUnlocked()) addKeyword(source, "Bane"); });
  ...
  if (name === "seria, gunslinger maid") consume(/Deal 1 damage to 2 random enemy followers\.?/i, () => { for (const unit of chooseRandomDistinct(enemyFollowers(),2)) damageUnit(unit,1,ctx.opponent,ctx.player,ctx,actions); });
  if (name === "okita souji") {
