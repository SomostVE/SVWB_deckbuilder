...
function useBonusPpIfUseful(player) {
...
function maybeEvolve(player, opponent, playerIndex, enemyIndex, stats, rng, map) {
  ...
  if (!candidates.length) return null;
  const wants = opponent.board.some(unit => unit.type === "Follower") || player.strategy.faceBias > .7 || opponent.hp <= 10;
  if (!wants) return null;
  const useSuper = player.personalTurn >= superTurn && player.sep > 0 && (opponent.board.length >= 2 || opponent.hp <= 8);
  const useNormal = player.personalTurn >= normalTurn && player.ep > 0;
  if (!useSuper && !useNormal) return null;
  const superMode = useSuper;
  const unit = [...candidates].sort((a,b)=>b.attack+b.defense-a.attack-a.defense)[0];
