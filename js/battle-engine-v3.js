...
export function analyzeCardSupport(card) {
  ...
  return recognized ? { level:"full", reason:"Covered by Battle Sim v3 rules", mechanics:[...mechanics] } : { level:"partial", reason:"Unique text is approximated", mechanics:[...mechanics] };
}
...
function drawCards(p,n,stats,idx){let d=0;for(let i=0;i<n;i++){if(!p.deck.length){p.deckOut=true;break;}const x=p.deck.shift();stats.draws[idx]++;d++;if(p.hand.length>=9){toCemetery(p,x,false);stats.cardsBurned[idx]++;}else p.hand.push(x);}return d;}
...
function playCard(inst,mode,p,o,pi,oi,stats,rng,cardMap){p.hand=p.hand.filter(x=>x.uid!==inst.uid);p.pp-=mode.cost;p.cardsPlayedThisTurn++;stats.cardsPlayed[pi]++;stats.ppSpent[pi]+=mode.cost;const c=inst.card,actions=[];let source=null;if(mode.kind!=="accelerate"){if(c.type==="Follower"){source=boardFollower(inst);p.board.push(source);p.rally++;}else if(c.type==="Amulet"){source=boardAmulet(inst);p.board.push(source);if((c.traits??[]).includes("Earth Sigil"))p.earthSigils++;}}
  ...
  if(c.type==="Spell"||mode.kind==="accelerate"){stats.spellsPlayed[pi]++;p.spellsPlayedThisTurn++;toCemetery(p,inst,true);spellboostHand(p,1,cardMap,actions);}
...
function spellboostHand(p,n,cardMap,actions=[]){for(let k=0;k<n;k++)for(const inst of p.hand){inst.spellboost=(+inst.spellboost||0)+1;const t=section(inst.card.text,"on spellboost");if(!t)continue;const xi=+(t.match(/Increase X by\s*(\d+)/i)?.[1]??0);if(xi)inst.x=(+inst.x||0)+xi;const stat=t.match(/give this follower\s*\+(\d+)\s*\/\s*\+(\d+)/i);if(stat){inst.attackBonus=(+inst.attackBonus||0)+ +stat[1];inst.defenseBonus=(+inst.defenseBonus||0)+ +stat[2];}const threshold=+(t.match(/if X is at least\s*(\d+)/i)?.[1]??Infinity);if(inst.x>=threshold&&/transform this card into/i.test(t)){const name=t.match(/transform this card into (?:an?\s+)?(.+?)(?:\.|$)/i)?.[1]?.trim();const target=name?findByName(cardMap,name):null;if(target)inst.card=target;}}}
...
function addHand(p,c,n,idx,stats){let k=0;for(let i=0;i<n;i++){const x=instance(p,c);if(p.hand.length>=9){toCemetery(p,x,false);stats.cardsBurned[idx]++;}else{p.hand.push(x);k++;}}return k;}
...
function attackPhase(p,o,pi,oi,stats,frames,players,round,rng,map,record){for(const atk of [...p.board].filter(u=>u.type==="Follower")){while(p.board.includes(atk)&&atk.attacksMade<atk.maxAttacks){const allWards=o.board.filter(u=>u.type==="Follower"&&hasU(u,"Ward"));const attackableWards=attackable(o.board).filter(u=>hasU(u,"Ward"));const foes=attackable(o.board);const canF=atk.canAttackFollower,canL=atk.canAttackLeader&&!allWards.length;let target=null,leader=false;if(allWards.length){if(canF&&attackableWards.length)target=tradeTarget(atk,attackableWards,p.strategy);else break;}else if(canL&&shouldFace(atk,p,o,foes,rng))leader=true;else if(canF&&foes.length)target=tradeTarget(atk,foes,p.strategy);else if(canL)leader=true;else break;atk.attacksMade++;atk.attacked=atk.attacksMade>=atk.maxAttacks;stats.attacks[pi]++;if(atk.ambush){atk.ambush=false;atk.keywords=atk.keywords.filter(k=>k!=="Ambush");}const acts=[];if(leader){const d=Math.max(0,atk.attack);o.hp-=d;stats.damageDealt[pi]+=d;if(hasU(atk,"Drain"))heal(p,d,stats,pi,acts);acts.push(...strike(atk,p,o,pi,oi,stats,rng,map));snap(frames,players,{round,active:pi,phase:"attack",action:compact(`${atk.name} attacks ${o.name}'s leader for ${d}.`,acts)},stats,record);if(o.hp<=0)return;continue;}
...
function cleanup(p,o,pi,oi,stats,rng,map){const a=[];let guard=0;while(guard++<12){const dead=p.board.filter(u=>u.type==="Follower"&&u.defense<=0);if(!dead.length)break;for(const u of dead){p.board=p.board.filter(x=>x.uid!==u.uid);toCemetery(p,{uid:u.uid,card:u.card},true);p.destroyedFollowers.push({card:u.card});stats.followersLost[pi]++;const lw=getTriggeredText(u.card,"lastWords");if(lw){stats.lastWordsTriggered[pi]++;const r=resolveText(lw,{card:u.card,sourceUnit:u,player:p,opponent:o,playerIndex:pi,enemyIndex:oi,stats,rng,cardMap:map});a.push(`${u.name} Last Words${r.actions.length?`: ${r.actions.join(" · ")}`:""}`);}}}return a;}
function destroyObject(p,o,u,pi,oi,stats,rng,map,last){p.board=p.board.filter(x=>x.uid!==u.uid);toCemetery(p,{uid:u.uid,card:u.card},true);if(u.type==="Follower")p.destroyedFollowers.push({card:u.card});if(!last)return[];const lw=getTriggeredText(u.card,"lastWords");if(!lw)return[];stats.lastWordsTriggered[pi]++;const r=resolveText(lw,{card:u.card,sourceUnit:u,player:p,opponent:o,playerIndex:pi,enemyIndex:oi,stats,rng,cardMap:map});return[`${u.name} Last Words`,...r.actions];}
function toCemetery(p,x,addShadow=false){p.cemetery.push(x);if(addShadow)p.shadows++;}
